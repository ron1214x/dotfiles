...
async function rebuild(context) {
  ...
  return new Promise(function(resolve, reject) {
    import_child_process2.default.execFile(cmd, args, opts, function(error) {
      if (error) {
...
async function activate(context) {
  ...
  const isVscodeOnigurumaDir = import_path2.default.join(context.extensionPath, "node_modules", "blade-formatter", "node_modules", "vscode-oniguruma");
  const binPath = extensionConfig.get("path", "");
  if (!binPath) {
    if (!import_fs2.default.existsSync(isVscodeOnigurumaDir)) {
  ...
  context.subscriptions.push(import_coc2.commands.registerCommand("bladeFormatter.rebuild", async () => {
    await rebuildWrapper(context);
  }));
}
async function rebuildWrapper(context) {
  let msg = "[blade-formatter] Run a build/rebuild of oniguruma?";
  let ret = 0;
  ret = await import_coc2.window.showQuickpick(["Yes", "Cancel"], msg);
  if (ret === 0) {
    ...
    try {
      ...
      const timerId = setInterval(() => {
        lap = new Date();
        import_coc2.window.showWarningMessage(`blade-formatter | oniguruma build... (${Math.floor((lap.getTime() - start.getTime()) / 1e3)} sec)`);
        if (isFinished) {
          const stop = new Date();
          import_coc2.window.showWarningMessage(`${msg} (${Math.floor((stop.getTime() - start.getTime()) / 1e3)} sec)`);
